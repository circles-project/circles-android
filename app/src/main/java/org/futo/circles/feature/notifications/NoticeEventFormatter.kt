...
class NoticeEventFormatter(
    ...
    private val currentUserId: String?
        get() = MatrixSessionProvider.currentSession?.myUserId
    ...
    fun format(timelineEvent: TimelineEvent, isDm: Boolean): CharSequence? {
        ...
        return when (val type = event.getClearType()) {
            ...
            EventType.STATE_ROOM_HISTORY_VISIBILITY -> formatRoomHistoryVisibilityEvent(
                event,
                senderName,
                isDm
            )
    ...
    private fun formatRoomPowerLevels(
        event: Event,
        disambiguatedDisplayName: String
    ): CharSequence? {
        val powerLevelsContent: PowerLevelsContent = event.content.toModel() ?: return null
        val previousPowerLevelsContent: PowerLevelsContent =
            event.resolvedPrevContent().toModel() ?: return null
        ...
        userIds.forEach { userId ->
            ...
            if (from != to) {
                ...
                val diff =
                    context.getString(R.string.notice_power_level_diff, userId, fromStr, toStr)
                diffs.add(diff)
            }
        }
        if (diffs.isEmpty()) {
        ...
        return if (event.isSentByCurrentUser()) {
            context.getString(R.string.notice_power_level_changed_by_you, diffStr)
        } else {
            context.getString(
                R.string.notice_power_level_changed,
        ...
        return if (widgetContent.isActive()) {
            ...
            if (previousWidgetContent?.isActive().orFalse()) {
                ...
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_widget_modified_by_you, widgetName)
                } else {
                    context.getString(
                        R.string.notice_widget_modified,
                        disambiguatedDisplayName,
                        widgetName
                    )
                }
                ...
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_widget_added_by_you, widgetName)
                } else {
                    context.getString(
                        R.string.notice_widget_added,
                        disambiguatedDisplayName,
                        widgetName
                    )
                }
            ...
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_widget_removed_by_you, widgetName)
            } else {
                context.getString(
                    R.string.notice_widget_removed,
                    disambiguatedDisplayName,
                    widgetName
                )
            }
    ...
    private fun formatRoomCreateEvent(event: Event, isDm: Boolean): CharSequence? {
        return event.content.toModel<RoomCreateContent>()
            ...
            ?.let {
                if (event.isSentByCurrentUser()) {
                    context.getString(if (isDm) R.string.notice_direct_room_created_by_you else R.string.notice_room_created_by_you)
                } else {
                    context.getString(
                        if (isDm) R.string.notice_direct_room_created else R.string.notice_room_created,
                        it.creator
                    )
                }
            }
    ...
    private fun formatRoomNameEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.name.isNullOrBlank()) {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_name_removed_by_you)
            } else {
                context.getString(R.string.notice_room_name_removed, senderName)
            }
        } else {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_name_changed_by_you, content.name)
            } else {
                context.getString(R.string.notice_room_name_changed, senderName, content.name)
            }
        }
    }
    ...
    private fun formatRoomTombstoneEvent(
        ...
        return if (event.isSentByCurrentUser()) {
            context.getString(if (isDm) R.string.notice_direct_room_update_by_you else R.string.notice_room_update_by_you)
        } else {
            context.getString(
                if (isDm) R.string.notice_direct_room_update else R.string.notice_room_update,
                senderName
            )
        }
    }
    ...
    private fun formatRoomTopicEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.topic.isNullOrEmpty()) {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_topic_removed_by_you)
            } else {
                context.getString(R.string.notice_room_topic_removed, senderName)
            }
        } else {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_topic_changed_by_you, content.topic)
            } else {
                context.getString(R.string.notice_room_topic_changed, senderName, content.topic)
            }
        }
    }
    ...
    private fun formatRoomAvatarEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return if (content.avatarUrl.isNullOrEmpty()) {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_avatar_removed_by_you)
            } else {
                context.getString(R.string.notice_room_avatar_removed, senderName)
            }
        } else {
            if (event.isSentByCurrentUser()) {
                context.getString(R.string.notice_room_avatar_changed_by_you)
            } else {
                context.getString(R.string.notice_room_avatar_changed, senderName)
            }
        }
    }

    private fun formatRoomHistoryVisibilityEvent(
        event: Event,
        senderName: String?,
        isDm: Boolean
    ): CharSequence? {
        val historyVisibility =
            event.content.toModel<RoomHistoryVisibilityContent>()?.historyVisibility ?: return null

        val historyVisibilitySuffix =
            roomHistoryVisibilityFormatter.getNoticeSuffix(historyVisibility)
        return if (event.isSentByCurrentUser()) {
            context.getString(
    ...
    private fun formatRoomThirdPartyInvite(
        event: Event,
        senderName: String?,
        isDm: Boolean
    ): CharSequence? {
        ...
        return when (type) {
            EventType.CALL_INVITE -> {
                ...
                return if (isVideoCall) {
                    if (event.isSentByCurrentUser()) {
                        context.getString(R.string.notice_placed_video_call_by_you)
                    } else {
                        context.getString(R.string.notice_placed_video_call, senderName)
                    }
                } else {
                    if (event.isSentByCurrentUser()) {
                        context.getString(R.string.notice_placed_voice_call_by_you)
                    } else {
                        context.getString(R.string.notice_placed_voice_call, senderName)
                    }
                }
            }
            EventType.CALL_ANSWER ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_answered_call_by_you)
                } else {
                    context.getString(R.string.notice_answered_call, senderName)
                }
            EventType.CALL_HANGUP ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_ended_call_by_you)
                } else {
                    context.getString(R.string.notice_ended_call, senderName)
                }
            EventType.CALL_CANDIDATES ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_call_candidates_by_you)
                } else {
                    context.getString(R.string.notice_call_candidates, senderName)
                }
            EventType.CALL_REJECT ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.call_tile_you_declined_this_call)
                } else {
                    context.getString(R.string.call_tile_other_declined, senderName)
                }
            else -> null
        }
    }
    ...
    private fun formatRoomAliasesEvent(event: Event, senderName: String?): String? {
        ...
        return when {
            addedAliases.isNotEmpty() && removedAliases.isNotEmpty() ->
                if (event.isSentByCurrentUser()) {
                    context.getString(
                        R.string.notice_room_aliases_added_and_removed_by_you,
                        addedAliases.joinToString(),
                        removedAliases.joinToString()
                    )
                } else {
                    context.getString(
                        R.string.notice_room_aliases_added_and_removed,
                        senderName,
                        addedAliases.joinToString(),
                        removedAliases.joinToString()
                    )
            ...
            addedAliases.isNotEmpty() ->
                if (event.isSentByCurrentUser()) {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_aliases_added_by_you,
                        addedAliases.size,
                        addedAliases.joinToString()
                    )
                } else {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_aliases_added,
                        addedAliases.size,
                        senderName,
                        addedAliases.joinToString()
                    )
            ...
            removedAliases.isNotEmpty() ->
                if (event.isSentByCurrentUser()) {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_aliases_removed_by_you,
                        removedAliases.size,
                        removedAliases.joinToString()
                    )
                } else {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_aliases_removed,
                        removedAliases.size,
                        senderName,
                        removedAliases.joinToString()
                    )
        ...
        return buildString {
            ...
            append(
                if (prevEventContent == null) {
                    if (event.isSentByCurrentUser()) {
                        context.getString(R.string.notice_room_server_acl_set_title_by_you)
                    } else {
                        context.getString(R.string.notice_room_server_acl_set_title, senderName)
                    }
                } else {
                    if (event.isSentByCurrentUser()) {
                        context.getString(R.string.notice_room_server_acl_updated_title_by_you)
                    } else {
                        context.getString(R.string.notice_room_server_acl_updated_title, senderName)
                    }
                }
            )
            if (eventContent.allowList.isEmpty()) {
                // Special case for stuck room
                appendNl(context.getString(R.string.notice_room_server_acl_allow_is_empty))
            }
        }
    }

    private fun StringBuilder.appendAclDetails(
        eventContent: RoomServerAclContent,
        prevEventContent: RoomServerAclContent?
    ) {
        if (prevEventContent == null) {
            eventContent.allowList.forEach {
                appendNl(
                    context.getString(
                        R.string.notice_room_server_acl_set_allowed,
                        it
                    )
                )
            }
            eventContent.denyList.forEach {
                appendNl(
                    context.getString(
                        R.string.notice_room_server_acl_set_banned,
                        it
                    )
                )
            }
            if (eventContent.allowIpLiterals) {
                appendNl(context.getString(R.string.notice_room_server_acl_set_ip_literals_allowed))
            } else {
                appendNl(context.getString(R.string.notice_room_server_acl_set_ip_literals_not_allowed))
            }
        } else {
            // Display only diff
            ...
            (eventContent.allowList - prevEventContent.allowList)
                ...
                .forEach {
                    appendNl(
                        context.getString(
                            R.string.notice_room_server_acl_updated_allowed,
                            it
                        )
                    )
                }
            ...
            (prevEventContent.allowList - eventContent.allowList)
                ...
                .forEach {
                    appendNl(
                        context.getString(
                            R.string.notice_room_server_acl_updated_was_allowed,
                            it
                        )
                    )
                }
            ...
            (eventContent.denyList - prevEventContent.denyList)
                ...
                .forEach {
                    appendNl(
                        context.getString(
                            R.string.notice_room_server_acl_updated_banned,
                            it
                        )
                    )
                }
            ...
            (prevEventContent.denyList - eventContent.denyList)
                ...
                .forEach {
                    appendNl(
                        context.getString(
                            R.string.notice_room_server_acl_updated_was_banned,
                            it
                        )
                    )
                }
            ...
            if (prevEventContent.allowIpLiterals != eventContent.allowIpLiterals) {
                hasChanged = true
                if (eventContent.allowIpLiterals) {
                    appendNl(context.getString(R.string.notice_room_server_acl_updated_ip_literals_allowed))
                } else {
                    appendNl(context.getString(R.string.notice_room_server_acl_updated_ip_literals_not_allowed))
                }
            }

            if (!hasChanged) {
                appendNl(context.getString(R.string.notice_room_server_acl_updated_no_change))
            }
        }
    }
    ...
    private fun formatRoomCanonicalAliasEvent(event: Event, senderName: String?): String? {
        ...
        return when {
            added.isEmpty() && removed.isEmpty() && canonicalAlias == prevCanonicalAlias -> {
                ...
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_room_canonical_alias_no_change_by_you)
                } else {
                    context.getString(R.string.notice_room_canonical_alias_no_change, senderName)
                }
            }
            added.isEmpty() && removed.isEmpty() -> {
                ...
                if (canonicalAlias != null) {
                    if (event.isSentByCurrentUser()) {
                        context.getString(
                            R.string.notice_room_canonical_alias_set_by_you,
                            canonicalAlias
                        )
                    } else {
                        context.getString(
                            R.string.notice_room_canonical_alias_set,
                            senderName,
                            canonicalAlias
                        )
                    }
                    ...
                    if (event.isSentByCurrentUser()) {
                        context.getString(R.string.notice_room_canonical_alias_unset_by_you)
                    } else {
                        context.getString(R.string.notice_room_canonical_alias_unset, senderName)
                    }
                }
            }
            added.isEmpty() && canonicalAlias == prevCanonicalAlias -> {
                ...
                if (event.isSentByCurrentUser()) {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_canonical_alias_alternative_removed_by_you,
                        removed.size,
                        removed.joinToString()
                    )
                } else {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_canonical_alias_alternative_removed,
                        removed.size,
                        senderName,
                        removed.joinToString()
                    )
            ...
            removed.isEmpty() && canonicalAlias == prevCanonicalAlias -> {
                ...
                if (event.isSentByCurrentUser()) {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_canonical_alias_alternative_added_by_you,
                        added.size,
                        added.joinToString()
                    )
                } else {
                    context.resources.getQuantityString(
                        R.plurals.notice_room_canonical_alias_alternative_added,
                        added.size,
                        senderName,
                        added.joinToString()
                    )
            ...
            canonicalAlias == prevCanonicalAlias -> {
                ...
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_room_canonical_alias_alternative_changed_by_you)
                } else {
                    context.getString(
                        R.string.notice_room_canonical_alias_alternative_changed,
                        senderName
                    )
                }
            }
                ...
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_room_canonical_alias_main_and_alternative_changed_by_you)
                } else {
                    context.getString(
                        R.string.notice_room_canonical_alias_main_and_alternative_changed,
                        senderName
                    )
                }
            }
    ...
    private fun formatRoomGuestAccessEvent(
        event: Event,
        senderName: String?,
        isDm: Boolean
    ): String? {
    ...
    private fun formatRoomEncryptionEvent(event: Event, senderName: String?): CharSequence? {
        ...
        return when (content.algorithm) {
            MXCRYPTO_ALGORITHM_MEGOLM ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_end_to_end_ok_by_you)
                } else {
                    context.getString(R.string.notice_end_to_end_ok, senderName)
                }
            else ->
                if (event.isSentByCurrentUser()) {
                    context.getString(
                        R.string.notice_end_to_end_unknown_algorithm_by_you,
                        content.algorithm
                    )
                } else {
                    context.getString(
                        R.string.notice_end_to_end_unknown_algorithm,
                        senderName,
                        content.algorithm
                    )
                }
    ...
    private fun buildProfileNotice(
        event: Event,
        senderName: String?,
        ...
        if (eventContent?.displayName != prevEventContent?.displayName) {
            val displayNameText = when {
                prevEventContent?.displayName.isNullOrEmpty() ->
                    if (event.isSentByCurrentUser()) {
                        context.getString(
                            R.string.notice_display_name_set_by_you,
                            eventContent?.displayName
                        )
                    } else {
                        context.getString(
                            R.string.notice_display_name_set,
                            event.senderId,
                            eventContent?.displayName
                        )
                    }
                eventContent?.displayName.isNullOrEmpty() ->
                    if (event.isSentByCurrentUser()) {
                        context.getString(
                            R.string.notice_display_name_removed_by_you,
                            prevEventContent?.displayName
                        )
                    } else {
                        context.getString(
                            R.string.notice_display_name_removed,
                            event.senderId,
                            prevEventContent?.displayName
                        )
                    }
                    ...
                    if (event.isSentByCurrentUser()) {
                        context.getString(
                            R.string.notice_display_name_changed_from_by_you,
                            prevEventContent?.displayName,
                            eventContent?.displayName
                        )
                    } else {
                        context.getString(
                            R.string.notice_display_name_changed_from,
                            event.senderId,
                            prevEventContent?.displayName,
                            eventContent?.displayName
                        )
        ...
        if (eventContent?.avatarUrl != prevEventContent?.avatarUrl) {
            val displayAvatarText = if (displayText.isNotEmpty()) {
                displayText.append(" ")
                context.getString(R.string.notice_avatar_changed_too)
            } else {
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_avatar_url_changed_by_you)
                } else {
                    context.getString(R.string.notice_avatar_url_changed, senderName)
                }
            }
        ...
        if (displayText.isEmpty()) {
            displayText.append(
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.notice_member_no_changes_by_you)
                } else {
                    context.getString(R.string.notice_member_no_changes, senderName)
                }
            )
        }
        ...
        return when (eventContent?.membership) {
            Membership.INVITE -> {
                when {
                    eventContent.thirdPartyInvite != null -> {
                        val userWhoHasAccepted =
                            eventContent.thirdPartyInvite?.signed?.mxid ?: event.stateKey
                        val threePidDisplayName = eventContent.thirdPartyInvite?.displayName ?: ""
                        eventContent.safeReason?.let { reason ->
                            if (event.isSentByCurrentUser()) {
                                context.getString(
                                    R.string.notice_room_third_party_registered_invite_with_reason_by_you,
                                    threePidDisplayName,
                                    reason
                                )
                            } else {
                                context.getString(
                                    R.string.notice_room_third_party_registered_invite_with_reason,
                                    userWhoHasAccepted,
                                    threePidDisplayName,
                                    reason
                                )
                            ...
                            context.getString(
                                R.string.notice_room_third_party_registered_invite_by_you,
                                threePidDisplayName
                            )
                        } else {
                            context.getString(
                                R.string.notice_room_third_party_registered_invite,
                                userWhoHasAccepted,
                                threePidDisplayName
                            )
                        }
                    ...
                    event.stateKey == currentUserId ->
                        eventContent.safeReason?.let { reason ->
                            context.getString(
                                R.string.notice_room_invite_you_with_reason,
                                senderDisplayName,
                                reason
                            )
                        } ?: context.getString(R.string.notice_room_invite_you, senderDisplayName)
                    event.stateKey.isNullOrEmpty() ->
                        if (event.isSentByCurrentUser()) {
                            eventContent.safeReason?.let { reason ->
                                context.getString(
                                    R.string.notice_room_invite_no_invitee_with_reason_by_you,
                                    reason
                                )
                            } ?: context.getString(R.string.notice_room_invite_no_invitee_by_you)
                        } else {
                            eventContent.safeReason?.let { reason ->
                                context.getString(
                                    R.string.notice_room_invite_no_invitee_with_reason,
                                    senderDisplayName,
                                    reason
                                )
                            } ?: context.getString(
                                R.string.notice_room_invite_no_invitee,
                                senderDisplayName
                            )
                        }
                    else ->
                        if (event.isSentByCurrentUser()) {
                            eventContent.safeReason?.let { reason ->
                                context.getString(
                                    R.string.notice_room_invite_with_reason_by_you,
                                    targetDisplayName,
                                    reason
                                )
                            } ?: context.getString(
                                R.string.notice_room_invite_by_you,
                                targetDisplayName
                            )
                        } else {
                            eventContent.safeReason?.let { reason ->
                                context.getString(
                                    R.string.notice_room_invite_with_reason,
                                    senderDisplayName,
                                    targetDisplayName,
                                    reason
                                )
                            } ?: context.getString(
                                R.string.notice_room_invite,
                                senderDisplayName,
                                targetDisplayName
                            )
                        }
            ...
            Membership.JOIN ->
                eventContent.safeReason?.let { reason ->
                    ...
                    if (event.isSentByCurrentUser()) {
                        context.getString(if (isDm) R.string.notice_direct_room_join_by_you else R.string.notice_room_join_by_you)
                    } else {
                        context.getString(
                    ...
                    when (prevEventContent?.membership) {
                        Membership.INVITE ->
                            if (event.isSentByCurrentUser()) {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_reject_with_reason_by_you,
                                        reason
                                    )
                                } ?: context.getString(R.string.notice_room_reject_by_you)
                            } else {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_reject_with_reason,
                                        senderDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_reject,
                                    senderDisplayName
                                )
                            }
                        else ->
                            eventContent.safeReason?.let { reason ->
                                ...
                                if (event.isSentByCurrentUser()) {
                                    context.getString(if (isDm) R.string.notice_direct_room_leave_by_you else R.string.notice_room_leave_by_you)
                                } else {
                                    context.getString(
                    ...
                    when (prevEventContent?.membership) {
                        Membership.INVITE ->
                            if (event.isSentByCurrentUser()) {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_withdraw_with_reason_by_you,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_withdraw_by_you,
                                    targetDisplayName
                                )
                            } else {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_withdraw_with_reason,
                                        senderDisplayName,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_withdraw,
                                    senderDisplayName,
                                    targetDisplayName
                                )
                            }
                        ...
                        Membership.JOIN ->
                            if (event.isSentByCurrentUser()) {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_remove_with_reason_by_you,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_remove_by_you,
                                    targetDisplayName
                                )
                            } else {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_remove_with_reason,
                                        senderDisplayName,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_remove,
                                    senderDisplayName,
                                    targetDisplayName
                                )
                            }
                        Membership.BAN ->
                            if (event.isSentByCurrentUser()) {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_unban_with_reason_by_you,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_unban_by_you,
                                    targetDisplayName
                                )
                            } else {
                                eventContent.safeReason?.let { reason ->
                                    context.getString(
                                        R.string.notice_room_unban_with_reason,
                                        senderDisplayName,
                                        targetDisplayName,
                                        reason
                                    )
                                } ?: context.getString(
                                    R.string.notice_room_unban,
                                    senderDisplayName,
                                    targetDisplayName
                                )
                            }
            ...
            Membership.BAN ->
                if (event.isSentByCurrentUser()) {
                    eventContent.safeReason?.let {
                        context.getString(
                            R.string.notice_room_ban_with_reason_by_you,
                            targetDisplayName,
                            it
                        )
                    } ?: context.getString(R.string.notice_room_ban_by_you, targetDisplayName)
                } else {
                    eventContent.safeReason?.let {
                        context.getString(
                            R.string.notice_room_ban_with_reason,
                            senderDisplayName,
                            targetDisplayName,
                            it
                        )
                    } ?: context.getString(
                        R.string.notice_room_ban,
                        senderDisplayName,
                        targetDisplayName
                    )
                }
            Membership.KNOCK ->
                if (event.isSentByCurrentUser()) {
                    ...
                    } ?: context.getString(R.string.notice_room_remove_by_you, targetDisplayName)
                } else {
                    eventContent.safeReason?.let { reason ->
                        context.getString(
                            R.string.notice_room_remove_with_reason,
    ...
    private fun formatJoinRulesEvent(
        event: Event,
        senderName: String?,
        isDm: Boolean
    ): CharSequence? {
        ...
        return when (content.joinRules) {
            RoomJoinRules.INVITE ->
                if (event.isSentByCurrentUser()) {
                    context.getString(if (isDm) R.string.direct_room_join_rules_invite_by_you else R.string.room_join_rules_invite_by_you)
                } else {
                    context.getString(
            ...
            RoomJoinRules.PUBLIC ->
                if (event.isSentByCurrentUser()) {
                    context.getString(R.string.room_join_rules_public_by_you)
                } else {
                    context.getString(R.string.room_join_rules_public, senderName)
                }
            else -> null
        }
    }
